...
const Home = () => {
    ...
    useEffect(() => {
        axios.get('http://ec2-13-232-148-209.ap-south-1.compute.amazonaws.com:3001/get')
            .then(result => setTodos(result.data))
            .catch(err => console.log(err));
    }, []);

    const edit = (id) => {
        axios.put(`http://ec2-13-232-148-209.ap-south-1.compute.amazonaws.com:3001/edit/${id}`)
            .then(result => {
    ...
    const Update = (id, updatedTask) => {
        axios.put(`http://ec2-13-232-148-209.ap-south-1.compute.amazonaws.com:3001/update/${id}`, { task: updatedTask })
            .then(result => {
                console.log(result.data);
                const updatedTodos = todos.map(todo => {
                    if (todo._id === id) {
    ...
    const Hdelete = (id) => {
        axios.delete(`http://ec2-13-232-148-209.ap-south-1.compute.amazonaws.com:3001/delete/${id}`)
            .then(result => {
                console.log(result.data);
